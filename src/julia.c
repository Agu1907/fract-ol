...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/17 21:07:49 by keezgi            #+#    #+#             */
/*   Updated: 2025/04/17 21:10:15 by keezgi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_julia(t_fract *fract)
{
	...
	while (y < HEIGHT)
	{
		...
		{
			iter = 0;
			fract->z_im = ((fract->max_im - fract->min_im) * y / (double)HEIGHT
					+ fract->min_im) + fract->im_shifting;
			fract->z_re = ((fract->max_re - fract->min_re) * x / (double)WIDTH
